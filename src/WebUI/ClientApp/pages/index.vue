<script setup>
    import { BoltIcon, ChatBubbleBottomCenterTextIcon, GlobeAltIcon, ScaleIcon } from '@heroicons/vue/24/outline/index.js'
    // const { data } = await useFetch('https://gorest.co.in/public/v2/users')
    const { data } = await useFetch('/api/Exam')
    const features = [
        {
...
<template>
    <div>
        ...
        <div v-for="exam in data" :key="exam.id">
            <div>{{exam}}</div>
        </div>
    </div>
</template>